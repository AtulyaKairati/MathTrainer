...
public class Multiplication extends AppCompatActivity {
    ...
    public void generateQues(){
        ...
        quesText.setText(String.format("%d X %d", a, b));
        ...
        for (int i=0; i<4; i++)
        {
            if (i == locOfAns)
                ...
                int incorrectAns = rand.nextInt(range*range);   //range/2) + a*b - range/4;

                // Making Sure that IncorrectAns Is NOt Same as a*b
                while ( incorrectAns == a * b || answers.contains(Integer.toString( incorrectAns )))
                    incorrectAns = rand.nextInt(range*range);    //range/2) + a*b - range/4;
